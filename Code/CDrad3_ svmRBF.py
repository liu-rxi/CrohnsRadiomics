...
out_path = 'J:\\Summer-Students\\Output\\RBFSVM\\'
...
param_type = '01mm'
total_exp = 200 # repitions for bootstrapping - use low val for now, then increase to 1k for publication
    ...
    for repeat_idx in range(total_exp): 
        ...
        #    print(j)
            ...
            tuned_parameters = [{'kernel': ['rbf'], 
                                      ...
                                      'C':  np.logspace(-3,3,7) }]  # 
            clf_cv = GridSearchCV(svm.SVC(), tuned_parameters, 
                                scoring='accuracy', # scoring='roc_auc'
                                cv=n_fold)                    
            ...
            clf = svm.SVC(kernel='rbf',C= clf_cv.best_params_['C'], 
                          gamma= clf_cv.best_params_['gamma'])                    
...
# df_feat.to_excel(out_path + "%s_top_features-%s.xlsx" % (param_type, time_now))
