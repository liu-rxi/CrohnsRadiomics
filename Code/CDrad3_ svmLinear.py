...
out_path = 'J:\\Summer-Students\\Output\\LinearSVM\\'
...
score_avgs = [np.zeros(len(Norm_wall_features_pd) + len(Abnorm_wall_features_pd_1)) for i in range(len(feat_flags))] # avg scores for BAGGED model --> ROC curve


for f in range(len(feat_flags)):
    feat_flag = feat_flags[f]
    ...
    if feat_flag== 'wall':    # whole bowel, use all features
            feature_norm  = Norm_wall_features_pd.to_numpy()
            feature_abnorm  = Abnorm_wall_features_pd_1.to_numpy()
            feature_names = list(Norm_wall_features_pd)
    elif feat_flag== 'wallCore': #use only texture features
            feature_norm  = Norm_wallCore_features_pd.to_numpy()[:,14:]
            feature_abnorm  = Abnorm_wallCore_features_pd_1.to_numpy()[:,14:]     
            feature_names = list(Norm_wall_features_pd)[14:]
    elif feat_flag== 'fat': #use all features
            feature_norm  = Norm_fat_features_pd.to_numpy()
            feature_abnorm  = Abnorm_fat_features_pd_1.to_numpy()
            feature_names = list(Norm_wall_features_pd)  
    elif feat_flag== 'fatCore': #use only texture  features
            feature_norm  = Norm_fatCore_features_pd.to_numpy()[:,14:]
            feature_abnorm  = Abnorm_fatCore_features_pd_1.to_numpy()[:,14:]  
            feature_names = list(Norm_wall_features_pd)[14:]
    ...
    #        print(clf_cv.best_params_)    
        ...
        score_avgs[f] += y_score/total_exp
...
## new columns for each type for 1) top features and 2) their weights (8 cols)
    ...
    top_rank = pd.DataFrame(
        ...
        columns=['%s_%s_feat' % (feat_flag, param_type), '%s_%s_wt' % (feat_flag, param_type)])
...
pl_i = [1,0,3,2]
for i in pl_i: #range(len(feat_flags)):

    pl_fpr, pl_tpr, _ = roc_curve(y, score_avgs[i])
    pl_auc = round(roc_auc_score(y, score_avgs[i]), 3)
    
    plt.plot(pl_fpr, pl_tpr, label = "%s AUC: %s" % (feat_flags[i], pl_auc))


plt.xlabel("False positive rate")
plt.ylabel("True positive rate")
plt.title("Single Model Receiver Operating Characteristic")
plt.legend()

plt.savefig(out_path + "%s %s SINGLE_FIG %s %s.png" % (fname_prefix, param_type, time_now, total_exp), dpi=300)
